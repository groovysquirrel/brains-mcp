...
import { recordLLMMetrics } from '../utils/MetricsCollector';
...
/**
 * Calculates cost based on token usage and model cost data
 * Uses input and output token pricing from the model configuration
 */
export const calculateCost = (
...
): { cost: number; currency: string; pricing?: { input: number; output: number } } | undefined => {
  // If no model or cost data, return undefined
  if (!model?.costPerToken) {
    return undefined;
  }
  
  // Get cost structure from model based on connection type
  const costPerToken = model.costPerToken;
  const pricingType = connectionType === 'PROVISIONED' && costPerToken.provisioned 
    ? 'provisioned' 
    : 'onDemand';
  
  const pricingData = costPerToken[pricingType];
  
  // Ensure we have an object with input/output properties
  if (typeof pricingData !== 'object' || 
      !('input' in pricingData) || 
      !('output' in pricingData)) {
    logger.warn(`Invalid cost structure format for ${connectionType}`, { 
      model: model.modelId,
      connectionType,
      costStructure: JSON.stringify(costPerToken)
    });
    return undefined;
  }

  // Calculate costs using input and output rates
  const inputRate = pricingData.input;
  const outputRate = pricingData.output;
  ...
  return {
    cost: totalCost,
    currency: 'USD',
    // Include pricing rates in the response for database storage
    pricing: {
      input: inputRate,
      output: outputRate
    }
...
export const trackUsageMetrics = async (
  ...
  try {
    ...
    // Calculate cost if model has cost data
    const costInfo = calculateCost(tokensIn, tokensOut, model, connectionType);
